...
namespace ET
{
    ...
    public static class DlgRoleInfoSystem
    {
        ...
        public static void RegisterUIEvent(this DlgRoleInfo self)
        {
            ...
            {
                //生成角色
                long id = Game.Scene.GetComponent<AccountInfoComponent>().AccountId;
                if (Game.Scene.GetComponent<RoleInfoComponent>().Exit(id))
                {
        ...
        public static void UpdateRole(this DlgRoleInfo self)
        {
            ...
            long userId = Game.Scene.GetComponent<AccountInfoComponent>().AccountId;
            for (int i = self.listGORoleInfo.Count - 1; i >= 0; i--)
            {
                GameObject.Destroy(self.listGORoleInfo[i]);
            }
            self.listGORoleInfo.Clear();
            foreach (var info in Game.Scene.GetComponent<RoleInfoComponent>().dicRoleInfo)
            {
                ...
                go.GetComponentInChildren<Button>().enabled = userId == info.Key;
